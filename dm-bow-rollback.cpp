...
void relocate(Relocations &relocations, sector_t dest, sector_t source,
              int count) {
  ...
  relocations.erase(relocations.lower_bound(dest),
                    relocations.lower_bound(dest + count));
  // Add new elements
...
bool checkCollision(Used_Sectors &used_sectors, sector_t start, sector_t end) {
  ...
  if (first_overlap->second) {
    return true;
  } else if (second_overlap != used_sectors.end() &&
             second_overlap->first < end) {
    return true;
  }
  return false;
}
...
// If that entry is a log sector, set the magic to kPartialRestoreMagic and
// flush.
void restoreSector(int device_fd, Used_Sectors &used_sectors,
                   std::vector<char> &ls_buffer, log_entry *le,
...
std::vector<char> relocatedRead(int device_fd, Relocations const &relocations,
  ...
  for (uint32_t i = 0; i < size;
       i += block_size, sector += block_size / kSectorSize) {
    auto relocation = --relocations.upper_bound(sector);
    lseek64(device_fd,
            (sector + relocation->second - relocation->first) * kSectorSize,
            SEEK_SET);
